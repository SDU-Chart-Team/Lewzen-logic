...
void load(std::string js) {
    nlohmann::json j = nlohmann::json::parse(LewzenServer::Register::run(R"({"command": "load", "json": )" + js + R"(})"));
    std::vector<std::string> ids = j["returns"][0]["ids"];
    id.insert(id.end(), ids.begin(), ids.end());
}
...
void test_sl() {
    ...
    std::string js = R"({"comps":[{"alignment":{"horizental":1,"vertical":1},"children":null,"core_points":{"B":{"arrow":"","can_remove":false,"color":"","id":"B","is_virtual":false,"x":65.0,"y":120.0},"Control":{"arrow":"","can_remove":false,"color":"orange","id":"Control","is_virtual":false,"x":0.0,"y":10.0},"L":{"arrow":"","can_remove":false,"color":"","id":"L","is_virtual":false,"x":0.0,"y":60.0},"LB":{"arrow":"","can_remove":false,"color":"","id":"LB","is_virtual":false,"x":0.0,"y":120.0},"LT":{"arrow":"","can_remove":false,"color":"","id":"LT","is_virtual":false,"x":0.0,"y":0.0},"R":{"arrow":"","can_remove":false,"color":"","id":"R","is_virtual":false,"x":130.0,"y":60.0},"RB":{"arrow":"","can_remove":false,"color":"","id":"RB","is_virtual":false,"x":130.0,"y":120.0},"RT":{"arrow":"","can_remove":false,"color":"","id":"RT","is_virtual":false,"x":130.0,"y":0.0},"T":{"arrow":"","can_remove":false,"color":"","id":"T","is_virtual":false,"x":65.0,"y":0.0}},"html":"","modules":["Basics","Flippable","Rectangle","Rotatable","Scalable","Stylized","Writable"],"spacing":{"bottom":0.0,"global":10.0,"left":0.0,"right":0.0,"top":0.0},"style":"stroke: black; fill: white; stroke-width: 1px;","theta":0.0,"type":"isometric_cube"}],"indices":[0]})";
    load(js);
    //std::cout << LewzenServer::Register::run(R"({"command": "move_point", "id": ")" + id[0] + R"(", "pid": "RB", "dx": 30, "dy": 20})") << std::endl; // 移动关键点
    //std::cout << LewzenServer::Register::run(R"({"command": "move_point", "id": ")" + id[0] + R"(", "pid": "Control", "dx": 6, "dy": -10})") << std::endl; // 移动关键点
    std::cout << LewzenServer::Canvas::HTML() << std::endl;
    std::cout << "-------" << std::endl;
}
