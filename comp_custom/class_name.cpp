...
namespace LewzenServer
{
    ...
    void ClassName::init()
    {
        ...
        // 绑定图形属性
        std::function<const std::string()> _getPath = std::bind(&ClassName::getPath, this);
        SVGIPath->D.bind(_getPath);
    } // 拷贝
    ComponentAbstract &ClassName::operator=(const ComponentAbstract &comp)
    {
        // 拷贝父类
        Rectangle::operator=(comp);

        auto &p = dynamic_cast<const ClassName &>(comp);
        // 拷贝关键点位置
        *Control0 = *(p.Control0);
    }
    // 序列化，并记录已操作的
    void ClassName::serialize(json &j, std::vector<std::string> &processed)
    {
        // 父类序列化
        Rectangle::serialize(j, processed);
    }
    // 反序列化
    ComponentAbstract &ClassName::operator=(const json &j)
    {
        // 父类反序列化
    ...
    //// Basics虚接口
    void ClassName::moveCorePoint(const std::string &id, const double &dx, const double &dy)
    {
    ...
    ////ClassName interface
    const std::string ClassName::getPath() const
    {
