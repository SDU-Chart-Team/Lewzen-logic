...
namespace LewzenServer {
    ...
    void Sort::init() {
        ...
        SVGIG->add(SVGIPath);
        // 绑定图形属性
        std::function<const std::string()> _getPath = std::bind(&Sort::getPath, this);
        SVGIPath->D.bind(_getPath);
    }    // 拷贝
    ComponentAbstract &Sort::operator=(const ComponentAbstract &comp) {
        // 拷贝父类
        Rectangle::operator=(comp);

        auto &p = dynamic_cast<const Sort &>(comp); 
        return *this;
    }
    // 序列化，并记录已操作的
    void Sort::serialize(json &j, std::vector<std::string> &processed) {
        // 父类序列化
  ...
  ////Sort interface
    const std::string Sort::getPath() const {
        ...
        auto p4 = createPoint(getX()+getWidth()*0.5, getY()+getHeight());
